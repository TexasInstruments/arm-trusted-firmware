...
static void scmi_clock_attributes(struct scmi_msg *msg)
{
	...
	return_values.attributes |= BIT(SCMI_CLOCK_PARENT_GET_ENABLE_POS);

	return_values.clock_enable_delay = plat_scmi_clock_get_enable_delay(unsigned int agent_id,
									    unsigned int scmi_id);
...
static void scmi_clock_possible_parents_get(struct scmi_msg *msg)
{
	...
	if (status == SCMI_SUCCESS) {
		...
		unsigned int plat_possible_parents[PARENTS_ARRAY_SIZE_MAX / sizeof(unsigned int)];
		size_t max_nb = PARENTS_ARRAY_SIZE_MAX / sizeof(unsigned int);
		...
		if (status == SCMI_SUCCESS) {
			...
			p2a.num_parents_flags = ((uint8_t)rem_nb) << 24;
			p2a.num_parents_flags |= (uint8_t)ret_nb;
...
static void scmi_clock_config_get(struct scmi_msg *msg)
{
	...
	if (in_args->flags & SCMI_CLOCK_CONFIG_EXTENDED_GET_ENABLE_MASK) {
		p2a.extended_config_val = plat_scmi_clock_get_extended_config(msg->agent_id,
									      clock_id);
	}
...
static void scmi_clock_config_set(struct scmi_msg *msg)
{
	...
	if (in_args->attributes & SCMI_CLOCK_CONFIG_EXTENDED_SET_ENABLE_MASK) {

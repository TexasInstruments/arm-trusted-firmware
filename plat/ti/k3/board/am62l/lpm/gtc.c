...
int32_t k3_lpm_sleep_suspend_gtc(void)
{
	...
	/* disable GTC */
	reg = mmio_read_32(GTC_CFG1_BASE + GTC_CFG1_CNTCR);
	reg &= ~GTC_CFG1_CNTCR_EN;
	mmio_write_32(GTC_CFG1_BASE + GTC_CFG1_CNTCR, reg);

	lpm_rtc_read_time(&gtc.suspend);

	gtc.hdbg    = reg & GTC_CFG1_CNTCR_HDBG;
	gtc.pushevt = mmio_read_32(GTC_CFG0_BASE + GTC_CFG0_PUSHEVT);
	gtc.cnt_lo  = mmio_read_32(GTC_CFG1_BASE + GTC_CFG1_CNT_LO);
	gtc.cnt_hi  = mmio_read_32(GTC_CFG1_BASE + GTC_CFG1_CNT_HI);
	gtc.fid0    = mmio_read_32(GTC_CFG1_BASE + GTC_CFG1_CNTFID0);
...
int32_t k3_lpm_resume_gtc(void)
{
	...
	mmio_write_32(GTC_CFG0_BASE + GTC_CFG0_PUSHEVT, gtc.pushevt);
	mmio_write_32(GTC_CFG1_BASE + GTC_CFG1_CNTFID0, gtc.fid0);
	...
	mmio_write_32(GTC_CFG1_BASE + GTC_CFG1_CNT_LO, gtc.cnt_lo);
	mmio_write_32(GTC_CFG1_BASE + GTC_CFG1_CNT_HI, gtc.cnt_hi);

	/* enable GTC */
	mmio_write_32(GTC_CFG1_BASE + GTC_CFG1_CNTCR, gtc.hdbg | GTC_CFG1_CNTCR_EN);

...
#define pll_readl mmio_read_32
#define pll_writel mmio_write_32
...
__wkupsramfunc int32_t pll_restore(struct pll_raw_data *pll)
{
	...
	/* Unlock write access */
	pll_writel(pll->base + PLL_16FFT_LOCKKEY0_OFFSET, PLL_16FFT_LOCKKEY0_VALUE);
	pll_writel(pll->base + PLL_16FFT_LOCKKEY1_OFFSET, PLL_16FFT_LOCKKEY1_VALUE);
	...
	pll_writel(pll->base + PLL_16FFT_CTRL_OFFSET, ctrl);
	...
	for (i = 0U; i < 16U; i++) {
		...
		if (((1U << (i + 16U)) & cfg) != 0U) {
			pll_writel(pll->base + PLL_16FFT_HSDIV_CTRL_OFFSET + (i * 0x4U), pll->hsdiv[i]);
			
		}
	}

	pll_writel(pll->base + PLL_16FFT_FREQ_CTRL0_OFFSET, pll->freq_ctrl0);
	pll_writel(pll->base + PLL_16FFT_FREQ_CTRL1_OFFSET, pll->freq_ctrl1);
	pll_writel(pll->base + PLL_16FFT_DIV_CTRL_OFFSET, pll->div_ctrl);
	...
	if ((ctrl & PLL_16FFT_CTRL_BYPASS_EN) != 0U) {
		/*
		 * The PLL_CTRL BYPASS_EN shoUd be cleared to make sure the clocks are running
		...
		do {
			timeout--;
			pll_stat = pll_readl(pll->base + PLL_16FFT_STAT_OFFSET) & ((uint32_t) PLL_16FFT_STAT_LOCK);
...
__wkupsramfunc void pll_bypass_hsdivs(struct pll_raw_data *pll)
{
	...
	for (i = 0U; i < 16U; i++) {
		/* Read HSDIV output if present */
		if (((1U << (i + 16U)) & cfg) != 0U) {
			pll_writel(pll->base + PLL_16FFT_HSDIV_CTRL_OFFSET + (i * 0x4U), 0x8001);
		}
	}

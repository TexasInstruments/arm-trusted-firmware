...
static void lpsc_module_get_internal(struct device *dev, struct lpsc_module *module, bool use, bool ret);
static void lpsc_module_put_internal(struct device *dev, struct lpsc_module *module, bool use, bool ret);
	...
	if (0U == (get_psc_pd_data(dev, pd)->flags & PSC_PD_ALWAYSON)) {
		...
		if (i == 0) {
			...
			pm_trace(TRACE_PM_ACTION_PSC_TRANSITION_TIMEOUT | TRACE_PM_ACTION_FAIL,
				 (uint32_t) (((uint32_t) ((to_psc_drv_data(get_drv_data(dev)))->psc_idx) << TRACE_PM_VAL_PSC_SHIFT) |
				 ((uint16_t) psc_pd_idx(dev, pd) << TRACE_PM_VAL_PD_SHIFT) | TRACE_PM_VAL_PD_POS1));
		}
	}
}
...
uint32_t psc_pd_get_state(struct device *dev, struct psc_pd *pd)
{
	...
	if ((psc_read(dev, psc_ptstat) & BIT(psc_idx % 32U)) != 0U) {
		...
		state = (uint8_t) (psc_read(dev, PSC_PDSTAT((uint32_t) psc_idx)) & PDSTAT_STATE_MASK);
				   ...
				   bool		domain_reset)
{
	...
	if ((get_en || get_ret) && ((data->flags & LPSC_DEPENDS) != 0UL)) {
		...
		if (!depends_dev) {
			pm_trace(TRACE_PM_ACTION_PSC_INVALID_DEP_DATA | TRACE_PM_ACTION_FAIL,
				 ...
				 ((uint16_t) data->depends_psc_idx << TRACE_PM_VAL_PD_SHIFT) | TRACE_PM_VAL_PD_POS1));
		} else {
			/*
	...
	if ((put_en || put_ret) && ((data->flags & LPSC_DEPENDS) != 0UL)) {
		...
		if (!depends_dev) {
			pm_trace(TRACE_PM_ACTION_PSC_INVALID_DEP_DATA | TRACE_PM_ACTION_FAIL,
				 (uint32_t) (((uint32_t) psc->psc_idx << TRACE_PM_VAL_PSC_SHIFT) |
				 ((uint16_t) data->depends_psc_idx << TRACE_PM_VAL_PD_SHIFT) | TRACE_PM_VAL_PD_POS2));
...
static int32_t psc_initialize_modules(struct device *dev)
{
	...
	for (idx = 0U; idx < psc->module_count; idx++) {
		...
		i = PSC_TRANSITION_TIMEOUT;
		i--;
		while ((((psc_read(dev, PSC_MDSTAT((uint32_t) idx))) & MDSTAT_BUSY_MASK) != 0U) && (i != 0)) {
			i--;
		}
		if (i == 0) {
			pm_trace(TRACE_PM_ACTION_PSC_TRANSITION_TIMEOUT | TRACE_PM_ACTION_FAIL,
				 (uint32_t) ((((uint32_t) psc->psc_idx) << TRACE_PM_VAL_PSC_SHIFT) |
				 ((uint16_t) idx << TRACE_PM_VAL_PD_SHIFT) | TRACE_PM_VAL_PD_POS3));
		}

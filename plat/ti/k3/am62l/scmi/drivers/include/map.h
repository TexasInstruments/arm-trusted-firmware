 ...
 * @map_ptr: the typed map to search.
 * @member: the uint32_t to search for.
 *
 * Returns the node, or NULL if it isn't in the map (and sets errno = ENOENT).
 *
 * Example:
 *	struct map_node *node = uint32_tmap_get(&map_ptr, 32425);
 */
 ...
 * @map_ptr: the typed map to add to.
 ...
 * @map_ptr: the typed map to add to.
 * @member: the uint32_t to place in the map.
 * @newn: the allocated node.
 *
 ...
 *
 * @handle's prototype should be:
 *	bool @handle(uint32_t member, struct map_node *n, typeof(arg) arg)
 *
 * If @handle returns false, the iteration will stop.
 * You should not alter the map within the @handle function!
 */
 ...
 * @prefix: the prefix.
 *
 * This returns a pointer into @map_ptr, so don't alter @map while using

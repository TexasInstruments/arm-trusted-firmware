...
struct clk_drv {
	 ...
	 */
	uint32_t (*set_freq)(struct clk *clkp, uint32_t target_hz,
	 ...
	 */
	bool (*notify_freq)(struct clk *clkp, uint32_t parent_freq_hz,
			    bool query);
...
bool clk_notify_sibling_freq(struct clk *clkp, struct clk *parent,
...
uint32_t clk_set_freq(struct clk *clkp, uint32_t target_hz, uint32_t min_hz,
 ...
 * through the chosen clock IDs, and disable all clocks are that are turned on.
 * Then, clear the initialized and power up flag.
 *
 * \param pm_devgrp

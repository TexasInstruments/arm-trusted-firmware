...
static bool pll_16fft_pllm_valid(struct clk *clock_ptr __unused, uint32_t pllm,
				 bool is_frac);

static int32_t pll_16fft_bin(struct clk *clock_ptr __unused,
			     uint32_t plld __unused,
			     uint32_t pllm __unused, bool is_frac,
			     uint32_t clkod __unused);

static uint32_t pll_16fft_vco_fitness(struct clk *clock_ptr __unused,
				      uint32_t vco,
				      bool is_frac __unused);

static bool pll_16fft_clkod_valid(struct clk *clock_ptr __unused,
				  uint32_t clkod);
...
static void clk_pll_16fft_cal_option3(const struct clk_data_pll_16fft *pll)
{
	...
	writel(cal, (uint32_t) pll->base + (uint32_t) PLL_16FFT_CAL_CTRL(pll->idx));
...
static int32_t clk_pll_16fft_enable_pll(const struct clk_data_pll_16fft *pll)
{
	...
	if ((ctrl & PLL_16FFT_CTRL_PLL_EN) == 0U) {
		...
		writel(ctrl, pll->base + (uint32_t) PLL_16FFT_CTRL(pll->idx));
		udelay(1U); /* Wait 1us */
	}
...
static uint32_t clk_pll_16fft_get_freq_internal(struct clk *clock_ptr,
						uint32_t clkod)
{
				       ...
				       uint32_t clkod)
{
	...
	if (ret) {
		writel(ctrl, (uint32_t) pll->base + (uint32_t) PLL_16FFT_CTRL(pll->idx));
	}
							       ...
							       bool query, bool *changed)
{
	...
	data_pll = container_of(pll_clk_data->data, const struct clk_data_pll,
				data);
	pll = container_of(data_pll, const struct clk_data_pll_16fft, data_pll);
	data_div = container_of(pll_clk_data->data, const struct clk_data_div,
				data);
	...
	for (i = 0; data_pll->pll_entries[i] != PLL_TABLE_LAST; i++) {
		...
		clkod_plld = (uint32_t)soc_pll_table[data_pll->pll_entries[i]].plld *
			     (uint32_t)soc_pll_table[data_pll->pll_entries[i]].clkod;
...
static int32_t clk_pll_16fft_init_internal(struct clk *clock_ptr)
{
	 ...
	 * written value.
	 */
	writel((uint32_t) PLL_16FFT_LOCKKEY0_VALUE,
	       (uint32_t) pll->base + (uint32_t) PLL_16FFT_LOCKKEY0(pll->idx));
	writel((uint32_t) PLL_16FFT_LOCKKEY1_VALUE,
...
static bool clk_pll_16fft_postdiv_valid_div(struct clk *clock_ptr __unused,
					    uint32_t d)
{
...
static bool clk_pll_16fft_postdiv_set_div(struct clk *clock_ptr, uint32_t d)
{
	...
	if (pll_clk != NULL) {
		...
		writel(div_ctrl, pll->base + (uint32_t) PLL_16FFT_DIV_CTRL(pll->idx));
	}
					     ...
					     bool query, bool *changed)
{
	...
	if ((clock_data->flags & CLK_DATA_FLAG_MODIFY_PARENT_FREQ) != 0U) {
		...
		if (pll_clk != NULL) {
			 ...
			 */
			ret = clk_div_set_freq_static_parent(clock_ptr,
							     target_hz,
							     target_hz,
							     target_hz,
							     query,

...
uint32_t clk_get_div(struct clk *clkp)
{
	...
	if (clk_datap->type != CLK_TYPE_DIV) {
		...
		divp = container_of(clk_datap->drv, const struct clk_drv_div,
				    drv);
...
static uint32_t clk_div_set_freq_dyn_parent(struct clk *clkp, uint32_t target_hz,
...
uint32_t clk_div_set_freq_static_parent(struct clk *clkp, uint32_t target_hz,
					uint32_t min_hz, uint32_t max_hz,

...
static void lpsc_module_notify_suspend(struct device *dev, struct lpsc_module *mod)
{
	const struct psc_drv_data *psc = to_psc_drv_data(get_drv_data(dev));
	uint32_t i;
 ...
 */
static void lpsc_module_sync_state(struct device *dev, struct lpsc_module *module,
				   bool domain_reset)
{
	const struct psc_drv_data *psc = to_psc_drv_data(get_drv_data(dev));
	uint32_t mdctl;
...
uint32_t lpsc_module_get_state(struct device *dev, struct lpsc_module *module)
{
	uint32_t idx = lpsc_module_idx(dev, module);
	uint8_t state;
	uint32_t ret;
...
void lpsc_module_set_local_reset(struct device *dev, struct lpsc_module *module,
				 bool enable)
{
	const struct psc_drv_data *psc = to_psc_drv_data(get_drv_data(dev));
	uint32_t idx = lpsc_module_idx(dev, module);
...
void lpsc_module_set_module_reset(struct device *dev, struct lpsc_module *module,
				  bool enable)
{
	const struct psc_drv_data *psc = to_psc_drv_data(get_drv_data(dev));
	uint32_t idx = lpsc_module_idx(dev, module);
				     ...
				     bool use, bool ret)
{
	...
	if (use) {
		VERBOSE("MODULE_GET: psc_id=%d lpsc_id=%d use_count=%d\n",
			psc->psc_idx, lpsc_module_idx(dev, module),
			module->use_count);
		module->use_count++;
 ...
 * in memory state.
 */
static void psc_initialize_pds(struct device *dev)
{
	const struct psc_drv_data *psc = to_psc_drv_data(get_drv_data(dev));
	pd_idx_t idx;
 ...
 */
static void psc_initialize_modules(struct device *dev)
{
	const struct psc_drv_data *psc = to_psc_drv_data(get_drv_data(dev));
	lpsc_idx_t idx;
 ...
 */
static void psc_initialize_modules_finish(struct device *dev)
{
	const struct psc_drv_data *psc = to_psc_drv_data(get_drv_data(dev));
	lpsc_idx_t idx;

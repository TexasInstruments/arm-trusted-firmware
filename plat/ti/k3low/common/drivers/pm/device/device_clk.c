...
const struct dev_clk_data *get_dev_clk_data(struct device *dev,
					    dev_clk_idx_t idx)
{
	const struct dev_data *data = get_dev_data(dev);
	const struct devgroup *devgrp = dev_data_lookup_devgroup(data);
...
bool device_clk_get_hw_ssc(struct device *dev __maybe_unused,
			   dev_clk_idx_t clk_idx __maybe_unused)
{
	return false;
}

void device_clk_set_freq_change(struct device *dev, dev_clk_idx_t clk_idx,
				bool allow)
{
	struct dev_clk *dev_clkp = get_dev_clk(dev, clk_idx);
	struct clk *clkp;
...
void device_clk_set_input_term(struct device *dev, dev_clk_idx_t clk_idx,
			       bool enable)
{
	struct dev_clk *dev_clkp = get_dev_clk(dev, clk_idx);
	bool is_term;
	...
	if (!dev_clkp) {
		...
		is_term = ((dev_clkp->flags & DEV_CLK_FLAG_INPUT_TERM) != 0U);
		if (is_term == enable) {
			/* Do nothing  - return */
		} else {
...
bool device_clk_set_parent(struct device *dev, dev_clk_idx_t clk_idx,
...
dev_clk_idx_t device_clk_get_num_parents(struct device *dev,
				 ...
				 uint32_t max_freq_hz, bool query)
{
	...
	if (!done) {
		...
		if (clock_data->type == DEV_CLK_TABLE_TYPE_OUTPUT) {
			 ...
			 */
			ret_freq = clk_value_set_freq(parent, target_freq_hz,
...
bool device_clk_set_freq(struct device *dev, dev_clk_idx_t clk_idx,
			 uint32_t min_freq_hz, uint32_t target_freq_hz,
